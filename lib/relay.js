...
const { array: isArray } = require('is');
 ...
 * @param {Array<string>} keys 子属性名称数组
 * @returns {any} 属性值
 */
function getValue(ctx, key, ...keys) {
  return ctx[key] !== undefined && keys.length > 0 ? getValue(ctx[key], ...keys) : ctx[key];
}
...
function relayRequest(mappings, ctx, { headers }, overwrite) {
  ...
  for (let key of keys) {
    ...
    let value = getValue(ctx, ...mappings[key]);
    if (value !== undefined)
      headers[key] = value;
  }
...
function onHttpClientRequestRelay({ ctx, args }) {
  ...
  args.dataType = args.dataType || TYPE_JSON;
  args.contentType = args.contentType || TYPE_JSON;
